...
public final class CraftPacketHandshake extends PacketHandshake
{
	...
	public CraftPacketHandshake(final int protocol, final String hostNameOrIP,
	...
	private Object constructPacket_Bukkit()
	{
		...
		Method toEnumProtocol = ReflectionAPI.getMethod(
				ReflectionAPI.getNmsClass("EnumProtocol"), "a", int.class);
		ReflectionAPI.setValue(packet, nextProtocolTypeField,
				ReflectionAPI.invokeMethod(null, toEnumProtocol,
						this.nextProtocolType.getProtocolId()));
		return packet;
	}

	private Object constructPacket_Glowstone()
	{
		return ReflectionAPI.newInstance(packetConstructor, this.protocol,
				this.hostNameOrIP, this.port,
				this.nextProtocolType.getProtocolId());
	}
	...
	static
	{
		if (MineAPI.isGlowstone())
		{
			packetClass = ReflectionAPI.getClass(
					"net.glowstone.net.message.handshake.HandshakeMessage");
			packetConstructor = ReflectionAPI.getConstructor(packetClass,
					int.class, String.class, int.class, int.class);
			protocolField = ReflectionAPI.getField(packetClass, "version",
					true);
			hostNameOrIPField = ReflectionAPI.getField(packetClass, "address",
					true);
			portField = ReflectionAPI.getField(packetClass, "port", true);
			nextProtocolTypeField = ReflectionAPI.getField(packetClass, "state",
					true);
		}
		else
		{
			packetClass = ReflectionAPI
					.getNmsClass("PacketHandshakingInSetProtocol");
			packetConstructor = ReflectionAPI.getConstructor(packetClass);
			protocolField = ReflectionAPI.getField(packetClass, "a", true);
			hostNameOrIPField = ReflectionAPI.getField(packetClass, "hostname",
					true);
			portField = ReflectionAPI.getField(packetClass, "port", true);
			nextProtocolTypeField = ReflectionAPI.getField(packetClass, "d",
					true);
		}
	}
