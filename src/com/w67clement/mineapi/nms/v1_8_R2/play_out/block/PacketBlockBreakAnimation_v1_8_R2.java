...
/**
 * Packet block break animation for 1.8.R2
 * 
 * @author w67clement
 * 
 */
public class PacketBlockBreakAnimation_v1_8_R2 implements PacketBlockBreakAnimation
{
	...
	public PacketBlockBreakAnimation_v1_8_R2(MC_Player player, Location blockLocation, byte destroyStage) {
		this.entityId = player.getEntityId();
		this.blockLocation = blockLocation;
		this.destroyStage = destroyStage;
	}

	public PacketBlockBreakAnimation_v1_8_R2(MC_Player player, int x, int y, int z, byte destroyStage) {
		this(player, new Location(Bukkit.getWorlds().get(0), x, y, z), destroyStage);
	}

	public PacketBlockBreakAnimation_v1_8_R2(Player player, Location blockLocation, byte destroyStage) {
		this(MineAPI.getNmsManager().getMCPlayer(player), blockLocation, destroyStage);
	}

	public PacketBlockBreakAnimation_v1_8_R2(Player player, int x, int y, int z, byte destroyStage) {
		this(MineAPI.getNmsManager().getMCPlayer(player), new Location(Bukkit.getWorlds().get(0), x, y, z),
				destroyStage);
	}
	...
	{
		PacketPlayOutBlockBreakAnimation packet = new PacketPlayOutBlockBreakAnimation(this.entityId,
				(BlockPosition) BlockPositionWrapper.fromLocation(this.blockLocation).toBlockPosition(),
				this.destroyStage);
