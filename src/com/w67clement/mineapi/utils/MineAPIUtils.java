...
public class MineAPIUtils
{
	...
	private static final String MINEAPIUTILS_PREFIX = ChatColor.GRAY + "[" + ChatColor.DARK_AQUA + "MineAPI"
			+ ChatColor.DARK_GREEN + "Utils" + ChatColor.GRAY + "]" + ChatColor.RESET + " ";

	public static final boolean isOnline(final String... urls) throws MalformedURLException
	{
		for (final String url : urls)
		{
			if (isOnline(new URL(url))) { return true; }
		}
		return false;
	}
	...
	public static boolean download(final String webSite, final File dest)
	{
		...
		{
			MineAPI.console
					.sendMessage(MINEAPIUTILS_PREFIX + ChatColor.GREEN + "Starting download from " + webSite + "...");
			if (!isOnline(webSite))
			{
				MineAPI.console.sendMessage(MINEAPIUTILS_PREFIX + ChatColor.DARK_RED + "[Error]" + ChatColor.RED
						+ " Can't download from: " + webSite + " Reason: NoConnection");
				return false;
			}
			...
			MineAPI.console.sendMessage(
					MINEAPIUTILS_PREFIX + ChatColor.GREEN + "The download from " + webSite + " has finished success");
			return true;
		}
