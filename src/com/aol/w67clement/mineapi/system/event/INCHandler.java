...
public class INCHandler implements IHandler {
	...
	private static Channel getChannel(Object networkManager) {
		...
		if (MineAPI.getServerVersion().equals("v1_8_R1")) {
			channel = (Channel) ReflectionAPI
					.getValue(networkManager, ReflectionAPI.getField(
							networkManager.getClass(), "i", true));
		} else if (MineAPI.getServerVersion().equals("v1_8_R2")) {
			channel = ((net.minecraft.server.v1_8_R2.NetworkManager) networkManager).k;
		} else if (MineAPI.getServerVersion().equals("v1_8_R3")) {
			channel = ((net.minecraft.server.v1_8_R3.NetworkManager) networkManager).channel;
		}
		return channel;
	}
	...
	public void addServerConnectionChannel() {
		...
		Object mc_Server = ReflectionAPI
				.invokeMethod(server, ReflectionAPI.getMethod(server,
						"getServer", new Class<?>[] {}), new Object[] {});
		...
		Field field = ReflectionAPI.getLastFieldOfType(serverConnectionClass,
				List.class);
		List<?> currentList = (List<?>) ReflectionAPI.getValue(
				serverConnection, field);
		Field field1 = ReflectionAPI.getField(currentList.getClass()
				.getSuperclass(), "list", true);
	...
	public class PingChannelHandler extends ChannelDuplexHandler {
		...
		public void write(ChannelHandlerContext ctx, Object msg,
			...
			PacketCancellable cancel = new PacketCancellable();
			mineapi.pingPacketSend(new PacketWrapper(msg), cancel, "Unknown IP");
			if (cancel.isCancelled()) {
				return;
			}
			super.write(ctx, msg, promise);
