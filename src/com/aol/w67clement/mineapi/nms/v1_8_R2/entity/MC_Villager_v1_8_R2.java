...
import com.aol.w67clement.mineapi.entity.villager.MC_MerchantRecipe;
import com.aol.w67clement.mineapi.entity.villager.MC_Villager;

public class MC_Villager_v1_8_R2 extends MC_EntityAgeable_v1_8_R2 implements
		MC_Villager {

	private EntityVillager villager;

	public MC_Villager_v1_8_R2(Villager entity) {
		super(entity);
	...
	public void addMerchantRecipe(MC_MerchantRecipe recipe) {
		Field field = ReflectionAPI.getField(this.villager.getClass(), "br",
				true);
		MerchantRecipeList recipes = (MerchantRecipeList) ReflectionAPI
				.getValue(this.villager, field);
	...
	public void setMerchantRecipe(List<MC_MerchantRecipe> recipes) {
		...
		ReflectionAPI.setValue(this.villager,
	...
	public void setMerchantRecipe(int index, MC_MerchantRecipe recipe) {
		...
		if (index > recipes.size()) {
			recipes.set(recipes.size(),
					(MerchantRecipe) recipe.toMerchantRecipeNms());
		} else {
	...
	public List<MC_MerchantRecipe> getMerchantRecipeList() {
		...
		MerchantRecipeList nms_recipes = (MerchantRecipeList) ReflectionAPI
				.getValue(this.villager, ReflectionAPI.getField(
						this.villager.getClass(), "br", true));
	...
	public MC_MerchantRecipe getMerchantRecipe(int index) {
		try {
			return new MC_MerchantRecipe(
					((MerchantRecipeList) ReflectionAPI.getValue(this.villager,
							ReflectionAPI.getField(this.villager.getClass(),
									"br", true))).get(index));
		} catch (Exception e) {
			e.printStackTrace();
		}
