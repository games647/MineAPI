...
public class ProtocolManager_v1_8_R2 implements ProtocolManager {
	...
	public ProtocolManager_v1_8_R2(MineAPI mineapi) {
		...
		try {
			...
			q = MinecraftServer.class.getDeclaredField("q");
			q.setAccessible(true);
			h = ServerConnection.class.getDeclaredField("h");
	...
	public void onPing(ServerListPingEvent event)
		...
		MinecraftServer server = (MinecraftServer) getConsole.get(this.mineapi
				.getServer());
		ServerConnection connection = (ServerConnection) this.q.get(server);
		@SuppressWarnings("unchecked")
		List<NetworkManager> list = (List<NetworkManager>) h.get(connection);
	...
	private void injectPlayer(final Player p) {
		try {
			...
			Channel channel = ep.playerConnection.networkManager.k;
	...
	private void injectConnection(NetworkManager manager)
			throws IllegalArgumentException, IllegalAccessException {
		final Channel channel = manager.k;
