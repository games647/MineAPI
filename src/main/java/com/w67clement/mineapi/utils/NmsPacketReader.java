...
public class NmsPacketReader
{
    ...
    public <U extends NmsPacket> U readPacket(Object packet, Class<? extends U> mineapi_packet)
    {
        if (injectReader == null)
        {
            ...
            {
                if (method.getName().equals("readPacket_" + mineapi_packet.getSimpleName()))
                {
                    hasReader = true;
                    break;
                }
            ...
            if (hasReader)
                return ReflectionAPI.invokeMethodWithType(this, ReflectionAPI.getMethod(this.getClass(), "readPacket_" + mineapi_packet.getSimpleName(), Object.class), mineapi_packet, packet);
            throw new RuntimeException("Cannot found reader for read '" + packet.getClass().getSimpleName() + "' for create instance of '" + mineapi_packet.getSimpleName() + "'.");
        }
        else
    ...
    public PacketHandshake readPacket_PacketHandshake(Object packet)
    {
        if (packet.getClass().getSimpleName().equals(PacketList.Handshake.getPacketName()) || PacketList.Handshake.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            else
            {
                ...
                int nextProtocolState = ReflectionAPI.invokeMethodWithType(protocolState, ReflectionAPI.getMethod(protocolState, "a"), int.class);
    ...
    public PacketPlayInChat readPacket_PacketPlayInChat(Object packet)
    {
        if (packet.getClass().getSimpleName().equals(PacketList.PacketPlayInChat.getPacketName()) || PacketList.PacketPlayInChat.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            if (MineAPI.isGlowstone())
            {
                minePacket.setMessage(ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "text", true), String.class));
            }
            else
            {
                minePacket.setMessage(ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "a", true), String.class));
            }
            return minePacket;
        }
        else
    ...
    public ClientCommand readPacket_ClientCommand(Object packet)
    {
        if (packet.getClass().getSimpleName().equals(PacketList.PacketPlayInClientCommand.getPacketName()) || PacketList.PacketPlayInClientCommand.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            if (MineAPI.isGlowstone())
            {
                int id = ReflectionAPI.getIntValue(packet, ReflectionAPI.getField(packet.getClass(), "action", true));
            ...
            {
                Object enum_constant = ReflectionAPI.getValue(packet, ReflectionAPI.getField(packet.getClass(), "a", true));
                int id = ReflectionAPI.invokeMethodWithType(enum_constant, ReflectionAPI.getMethod(enum_constant, "ordinal"), int.class);
                for (ClientCommandType type : ClientCommandType.values())
                {
                    if (id == type.getId())
                        minePacket = MineAPI.getNmsManager().getPacketPlayInClientCommand(type);
    ...
    public PacketExplosion readPacket_PacketExplosion(Object packet)
    {
        if (packet.getClass().getSimpleName().equals(PacketList.PacketPlayOutExplosion.getPacketName()) || PacketList.PacketPlayOutExplosion.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            if (MineAPI.isGlowstone())
            {
                ...
                double y = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "y", true), double.class);
                double z = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "z", true), double.class);
                float radius = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "radius", true), float.class);
                minePacket = MineAPI.getNmsManager().getExplosionPacket(null, x, y, z, radius, true);
            }
            else
            {
                double x = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "a", true), double.class);
                double y = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "b", true), double.class);
                double z = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "c", true), double.class);
                float radius = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "d", true), float.class);
                minePacket = MineAPI.getNmsManager().getExplosionPacket(null, x, y, z, radius, true);
            }
            return minePacket;
        }
        ...
        if (packet.getClass().getSimpleName().equals(PacketList.PacketPlayOutWindowItems.getPacketName()) || PacketList.PacketPlayOutWindowItems.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            else
            {
                ...
                for (int i = 0; i < itemsLength; i++)
                {
                    ...
                    contents.add(ReflectionAPI.ItemStackConverter.fromNms(nms_Item));
                }
                minePacket = MineAPI.getNmsManager().getWindowItemsPacket(windowId, contents);
            }
            return minePacket;
        }
        else
    ...
    public PacketStatusOutPong readPacket_PacketStatusOutPong(Object packet)
    {
        if (packet.getClass().getSimpleName().equals(PacketList.PacketStatusOutPong.getPacketName()) || PacketList.PacketStatusOutPong.getPacketAliases().contains(packet.getClass().getSimpleName()))
        {
            ...
            {
                long pong = ReflectionAPI.getValueWithType(packet, ReflectionAPI.getField(packet.getClass(), "a", true), long.class);
                minePacket = MineAPI.getNmsManager().getPacketStatusOutPong(pong);
            }
            return minePacket;
        }
